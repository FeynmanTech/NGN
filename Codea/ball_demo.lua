...
function setup()
    cl.func.ellipse = function(lvars, x, y, r)
        ellipse(x, y, r)
    end
    cl.func.background = function(lvars, ...)
        background(...)
    end
    cs = [[
    $x = WIDTH/2;
...
cl.key["if.-{.-};%s-else"] = "if%s-([^{]-)%s-(%b{});%s-else%s-(%b{});"
cl.proc["if.-{.-};%s-else"] = function(lvars, conditions, src, src2)
    --lvars[var] = start
    if cl.isTrue(conditions, lvars) then
        cl.parse(src:sub(2, -2), lvars, true)
    else
